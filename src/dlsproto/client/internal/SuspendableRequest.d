/*******************************************************************************
    ...
    Helper struct for client-side suspendable requests which behave as follows:
        ...
        * State changes are initiated by the user via a "controller" -- an
          ...
          progress.
        * The node may inform the client at any time (including when the request
          is suspended) that the request has ended.
        * The node may inform the client at any time (including when the request
          is suspended) that the request has ended.
...
module dlsproto.client.internal.SuspendableRequest;
...
struct SuspendableRequest
{
    ...
    ***************************************************************************/
    ...
    public enum State
    {
        ...
        /// Handling received Finish notification
    ...
    ***************************************************************************/
    ...
    public enum ReceivedMessageAction
    {
        ...
        /// The received message indicates that no more data will be sent and no
        /// more state changes will be performed on this connection
        Finished
    }
    ...
    ***************************************************************************/
        ...
        scope ReceivedMessageDg handle_received_message, scope void delegate ( ) state_changed )
    {
        ...
        try
        {
            ...
            // The request may finish while we're waiting for ACK
            bool request_finished;
            if ( !(&this).waitForAck(handle_received_message, request_finished) )
                return request_finished? State.HandlingFinishNotification : State.Exit;
        }
        finally
        {
            assert((&this).shared_working.waiting_for_ack_count);
    ...
    /***************************************************************************
        ...
        Handles the case where the node has informed the client that the request
        has finished. Sends an ACK to the node that the client has also ended
        the request and will not send any further messages.

        Params:
            ack = code for the Ack
    ...
    ***************************************************************************/
    ...
    public State handleFinishNotification ( ubyte ack )
    {
        (&this).conn.send(
            ( conn.Payload payload )
            {
                payload.addConstant(ack);
            }
        );
    ...
    /***************************************************************************
        ...
        Params:
            handle_received_message = delegate to which messages received while
                changing state are passed. The return value determines whether
                the request continues or exits.
            received_finish_notification = out parameter set to true in case
                `Finished` message was received instead of Ack.
    ...
    ***************************************************************************/
    ...
    private bool waitForAck (
        scope ReceivedMessageDg handle_received_message,
        out bool received_finish_notification )
    {
        bool ack;
        do
        {
            ...
            with ( ReceivedMessageAction ) switch ( msg_action )
            {
                case Exit:
                    return false;
                case Finished:
                    received_finish_notification = true;
                    return false;
                case Continue:
                    break;
                case Ack:
